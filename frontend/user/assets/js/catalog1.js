...
const fetchData = async (genre, label) => {
  try {
    const response = await axios.get(
      `http://localhost:8080/api/filter?genre=${genre}&label=${label}`,
      {
        headers: {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (
    (will_like === "" && label === "" && genre === "") ||
    (will_like === null && label === null && genre === null)
  ) {
    ...
    fetchData(genre, label).then((response) => {
      ...
      const clickItem = document.querySelectorAll(
    ...
    const now_watching = document.querySelector(
    ...
    fetchData(will_like, label).then((response) => {
      axios
        .get(
          `http://localhost:8080/api/movie?movieId=${localStorage.getItem(
            "movieid"
          )}`,
          {
            headers: {
