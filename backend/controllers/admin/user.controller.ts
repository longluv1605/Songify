...
class AdminUserController {
    ...
    public getUserData = async (req: AuthenticatedRequest, res: Response) => {
        try {
            const sql: string = `
                ...
                LEFT JOIN (SELECT user_id, COUNT(*) AS rating_count FROM user_rating GROUP BY user_id) r ON u.id = r.user_id LIMIT 100000
            `
            // console.log(sql);
            ...
            for (let user of users) {
                user.comment_count = parseInt(user.comment_count as string) || 0;
    ...
    public changeUserStatus = async (req: AuthenticatedRequest, res: Response) => {
        try {
            ...
            const data = await this.db.query(`SELECT status FROM user WHERE id = ?`, [userId]);

            if (data.length === 0) {
                res.status(300).json({ message: "User not found" });
            } else {
                let status = data[0].status === "accepted" ? "banned" : "accepted";
    ...
    public deleteUser = async (req: AuthenticatedRequest, res: Response) => {
        try {
            ...
            let sql: string = `DELETE FROM user_rating WHERE user_id = ?`
            await this.db.query(sql, [userId]);
            
            sql = `DELETE FROM user_plan WHERE user_id = ?`
            await this.db.query(sql, [userId]);
            ...
            sql = `DELETE FROM user_purchase WHERE id = ?`
            await this.db.query(sql, [userId]);

            sql = `DELETE FROM user_watchlist WHERE id = ?`
            await this.db.query(sql, [userId]);

            sql = `DELETE FROM user_favorite WHERE id = ?`
            await this.db.query(sql, [userId]);
