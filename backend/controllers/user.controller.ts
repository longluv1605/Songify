...
class UserController {
  ...
  public getHomeData = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const newMovies = await MovieManager.getRecommender().getNewMovies();
      const recentMovies =
        await MovieManager.getRecommender().getRecentlyViewedMovies(userId);
  ...
  public getMovieData = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const your_rating = await RatingManager.getDatas({ userId, movieId });

      // console.log("Movie data:", movieData);
      // console.log("Comments:", comments);
      res.status(200).json({ movieData, comments, your_rating });
    } catch (err) {
  ...
  public searchMovie = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const searchResults = await MovieManager.getSearcher().getMovieByTitle({
        searchQuery,
      });
  ...
  public filterMovie = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (label && genre) {
        filterResults = await MovieManager.getFilter().getMovieByGenreAndLabel({
          genre,
          label,
        });
      } else if (label) {
  ...
  public deleteMovieFromWatchlist = async (
    ...
    try {
      ...
      res
        .status(200)
        .json({ message: "Remove movie from watchlist successfully" });
    } catch (err) {
